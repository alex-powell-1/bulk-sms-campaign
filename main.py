...
def query_db(sql_query):
    try:
        ...
        messagebox.showerror(
            title="Network Error",
            message="Cannot connect to server. Check VPN settings.",
        )
            ...
            if customer["PHONE_1"] == "error":
                ...
                # Catch Errors
                    ...
                    if str(err)[-22:] == "is not a mobile number":
                        ...
                        customer["response_code"] = (
                            f"Code: {err.code} - No Permission to send SMS"
                        )
                        ...
                        customer["response_code"] = (
                            f"Code: {err.code} - Permission Denied. Check Auth Token"
                        )
                    else:
                        customer["response_code"] = (
                            f"Code: {err.code} - Unknown TwilioRestException"
                        )
