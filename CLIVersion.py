...
import creds_old
import custom
from creds_old import SERVER, DATABASE, USERNAME, PASSWORD, account_sid, auth_token
...
def send_text(recipients, sms_message, error_log, test_mode=False, photo_message=False, photo_url=''):
    ...
    for customer in recipients:
            ...
            try:
                if photo_message:
                    twilio_message = client.messages.create(
                        from_=creds_old.TWILIO_PHONE_NUMBER,
                        media_url=[photo_url],
                        to=customer['PHONE_1'],
                        body=custom_message,
                    )
                    ...
                    twilio_message = client.messages.create(
                        from_=creds_old.TWILIO_PHONE_NUMBER, to=customer['PHONE_1'], body=custom_message
                    )
        ...
        try:
            ...
            print(f'Log Error: {err}', file=creds_old.error_log)
...
def write_log(customer):
    ...
    try:
        open(creds_old.log_file_path, 'r')

    except FileNotFoundError:
        log_file = open(creds_old.log_file_path, 'a')
...
def mass_sms_campaign():
    message = (
        'Use this coupon to save $5 on any hanging baskets Thursday - Saturday! '
    ...
    send_text(
        recipients=read_csv(contact_list),
        sms_message=message,
        test_mode=True,
        photo_message=True,
        photo_url=photo_link,
        error_log=creds_old.error_log,
    )
